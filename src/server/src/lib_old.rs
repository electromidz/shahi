...
impl Server {
    pub async fn start_server() -> Result<(), Box<dyn Error>> {
        ...
        let key: Vec<u8> = fs::read("certs/server_key.pem").expect("Failed to load key.pem file!");
        let cert: Vec<u8> =
            fs::read("certs/server_cert.pem").expect("Failed to load cert.pem file!");
        ...
        while let Some(connecting) = server.accept().await {
            ...
            println!("{:?}  - {:?}", connection.stats(), connection.rtt());
        }

        // let transport_config = Arc::get_mut(&mut server_config.transport).unwrap();
        // transport_config.max_concurrent_uni_streams(0_u8.into());
        //
        // let addr: SocketAddr = "0.0.0.0:6969".parse()?; // Ensure address is set
...
async fn handle_connection(root: Router, conn: quinn::Incoming) -> Result<(), Box<dyn Error>> {
